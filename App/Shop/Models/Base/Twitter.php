 ...
 */
abstract class Twitter implements ActiveRecordInterface
{
    /**
     * TableMap class name
     *
     * @var string
     */
    public const TABLE_MAP = '\\Shop\\Models\\Map\\TwitterTableMap';
    ...
    /**
     * The value for the twitterapiid field.
     *
     * @var        string
     */
    protected $twitterapiid;
    ...
    /**
     * Initializes internal state of Shop\Models\Base\Twitter object.
     */
    ...
    /**
     * Compares this with another <code>Twitter</code> instance.  If
     * <code>obj</code> is an instance of <code>Twitter</code>, delegates to
     * <code>equals(Twitter)</code>.  Otherwise, returns <code>false</code>.
     *
    ...
    /**
     * Get the [twitterapiid] column value.
     *
     * @return string
     */
    public function getTwitterapiid()
    {
        return $this->twitterapiid;
    }
    ...
    public function setTwitterid($v)
    {
        ...
        if ($this->twitterid !== $v) {
            $this->twitterid = $v;
            $this->modifiedColumns[TwitterTableMap::COL_TWITTERID] = true;
        }
    ...
    /**
     * Set the value of [twitterapiid] column.
     *
     * @param string $v New value
     * @return $this The current object (for fluent API support)
     */
    public function setTwitterapiid($v)
    {
        if ($v !== null) {
            $v = (string) $v;
        }

        if ($this->twitterapiid !== $v) {
            $this->twitterapiid = $v;
            $this->modifiedColumns[TwitterTableMap::COL_TWITTERAPIID] = true;
        }
    ...
    public function setName($v)
    {
        ...
        if ($this->name !== $v) {
            $this->name = $v;
            $this->modifiedColumns[TwitterTableMap::COL_NAME] = true;
        }
    ...
    public function hydrate(array $row, int $startcol = 0, bool $rehydrate = false, string $indexType = TableMap::TYPE_NUM): int
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : TwitterTableMap::translateFieldName('Twitterid', TableMap::TYPE_PHPNAME, $indexType)];
            $this->twitterid = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : TwitterTableMap::translateFieldName('Twitterapiid', TableMap::TYPE_PHPNAME, $indexType)];
            $this->twitterapiid = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : TwitterTableMap::translateFieldName('Name', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 3; // 3 = TwitterTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\Shop\\Models\\Twitter'), 0, $e);
        }
    }
    ...
    public function reload(bool $deep = false, ?ConnectionInterface $con = null): void
    {
        ...
        $dataFetcher = ChildTwitterQuery::create(null, $this->buildPkeyCriteria())->setFormatter(ModelCriteria::FORMAT_STATEMENT)->find($con);
     ...
     * @return void
     * @throws \Propel\Runtime\Exception\PropelException
     * @see Twitter::setDeleted()
     * @see Twitter::isDeleted()
     */
    public function delete(?ConnectionInterface $con = null): void
    {
        ...
        $con->transaction(function () use ($con) {
            $deleteQuery = ChildTwitterQuery::create()
    ...
    public function save(?ConnectionInterface $con = null)
    {
        ...
        return $con->transaction(function () use ($con) {
            ...
            if ($ret) {
                ...
                $this->postSave($con);
                TwitterTableMap::addInstanceToPool($this);
            } else {
                $affectedRows = 0;
            }
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->twittersScheduledForDeletion !== null) {
                if (!$this->twittersScheduledForDeletion->isEmpty()) {
                    foreach ($this->twittersScheduledForDeletion as $twitter) {
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if (null !== $this->twitterid) {
            throw new PropelException('Cannot insert a value for auto-increment primary key (' . TwitterTableMap::COL_TWITTERID . ')');
        }

         // check the columns in natural order for more readable SQL queries
        if ($this->isColumnModified(TwitterTableMap::COL_TWITTERID)) {
            $modifiedColumns[':p' . $index++]  = 'twitterId';
        }
        if ($this->isColumnModified(TwitterTableMap::COL_TWITTERAPIID)) {
            $modifiedColumns[':p' . $index++]  = 'twitterApiId';
        }
        if ($this->isColumnModified(TwitterTableMap::COL_NAME)) {
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    case 'twitterId':
                        ...
                        break;
                    case 'twitterApiId':
                        $stmt->bindValue($identifier, $this->twitterapiid, PDO::PARAM_STR);
                        break;
                    case 'name':
    ...
    {
        $pos = TwitterTableMap::translateFieldName($name, $type, TableMap::TYPE_NUM);
    ...
    {
        if (isset($alreadyDumpedObjects['Twitter'][$this->hashCode()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['Twitter'][$this->hashCode()] = true;
        $keys = TwitterTableMap::getFieldNames($keyType);
        $result = [
            $keys[0] => $this->getTwitterid(),
            $keys[1] => $this->getTwitterapiid(),
            $keys[2] => $this->getName(),
        ];
    ...
    public function fromArray(array $arr, string $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[0], $arr)) {
            $this->setTwitterid($arr[$keys[0]]);
        }
        if (array_key_exists($keys[1], $arr)) {
            $this->setTwitterapiid($arr[$keys[1]]);
        }
    ...
    {
        $criteria = new Criteria(TwitterTableMap::DATABASE_NAME);

        if ($this->isColumnModified(TwitterTableMap::COL_TWITTERID)) {
            $criteria->add(TwitterTableMap::COL_TWITTERID, $this->twitterid);
        }
        if ($this->isColumnModified(TwitterTableMap::COL_TWITTERAPIID)) {
            $criteria->add(TwitterTableMap::COL_TWITTERAPIID, $this->twitterapiid);
        }
        if ($this->isColumnModified(TwitterTableMap::COL_NAME)) {
            $criteria->add(TwitterTableMap::COL_NAME, $this->name);
        }
    ...
    {
        $criteria = ChildTwitterQuery::create();
        $criteria->add(TwitterTableMap::COL_TWITTERID, $this->twitterid);
    ...
    {
        $validPk = null !== $this->getTwitterid();
    ...
    /**
     * Generic method to set the primary key (twitterid column).
     *
     ...
     * objects.
     *
     * @param      object $copyObj An object of \Shop\Models\Twitter (or compatible) type.
    ...
    {
        $copyObj->setTwitterapiid($this->getTwitterapiid());
        ...
        if ($makeNew) {
            $copyObj->setNew(true);
            $copyObj->setTwitterid(NULL); // this is a auto-increment column, so set to default value
        }
    }
     ...
     *
     * @param  bool $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Shop\Models\Twitter Clone of current object.
     * @throws \Propel\Runtime\Exception\PropelException
     */
    ...
    public function initTwitters($overrideExisting = true)
    {
        ...
        $this->collTwitters->setModel('\Shop\Models\User');
    }
     ...
     * If this ChildTwitter is new, it will return
        ...
        if (null === $this->collTwitters || null !== $criteria || $partial) {
            if ($this->isNew()) {
                ...
                if (null === $this->collTwitters) {
                    ...
                    $collTwitters = new $collectionClassName;
                    $collTwitters->setModel('\Shop\Models\User');

                    return $collTwitters;
                }
            } else {
                $collTwitters = ChildUserQuery::create(null, $criteria)
                    ->filterByTwitter($this)
                    ->find($con);
     ...
     * @param      ConnectionInterface $con Optional connection object
     * @return $this|ChildTwitter The current object (for fluent API support)
     */
    public function setTwitters(Collection $twitters, ?ConnectionInterface $con = null)
    {
        /** @var ChildUser[] $twittersToDelete */
        ...
        foreach ($twittersToDelete as $twitterRemoved) {
            $twitterRemoved->setTwitter(null);
        }
     ...
     * @return $this The current object (for fluent API support)
     */
    public function addTwitter(ChildUser $l)
    {
        if ($this->collTwitters === null) {
            $this->initTwitters();
            $this->collTwittersPartial = true;
    ...
    /**
     * @param ChildUser $twitter The ChildUser object to add.
     */
    protected function doAddTwitter(ChildUser $twitter)
    {
        $this->collTwitters[]= $twitter;
        $twitter->setTwitter($this);
    }

    /**
     * @param  ChildUser $twitter The ChildUser object to remove.
     * @return $this|ChildTwitter The current object (for fluent API support)
     */
    public function removeTwitter(ChildUser $twitter)
    {
        if ($this->getTwitters()->contains($twitter)) {
            ...
            $this->twittersScheduledForDeletion[]= $twitter;
    ...
    {
        return (string) $this->exportTo(TwitterTableMap::DEFAULT_STRING_FORMAT);
    }
