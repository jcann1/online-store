...
abstract class Product implements ActiveRecordInterface
{
     ...
     * @var string
     */
    public const TABLE_MAP = '\\Shop\\Models\\Map\\ProductTableMap';
     ...
     *
     * Note: this column has a database default value of: 'product/default/default_product.png'
     * @var        string
     */
    protected $imageurl;
    ...
    /**
     * @var        ObjectCollection|ChildPurchase[] Cross Collection to store aggregation of ChildPurchase objects.
     * @phpstan-var ObjectCollection&\Traversable<ChildPurchase> Cross Collection to store aggregation of ChildPurchase objects.
     */
    ...
    {
        $this->quantity = 0;
        $this->imageurl = 'product/default/default_product.png';
        $this->isdeleted = false;
    }

    /**
     * Initializes internal state of Shop\Models\Base\Product object.
     * @see applyDefaults()
     */
    ...
    public function setImageurl($v)
    {
        ...
        if ($this->imageurl !== $v) {
            $this->imageurl = $v;
            $this->modifiedColumns[ProductTableMap::COL_IMAGEURL] = true;
        }
    ...
    public function setIsdeleted($v)
    {
        ...
        if ($this->isdeleted !== $v) {
            $this->isdeleted = $v;
            $this->modifiedColumns[ProductTableMap::COL_ISDELETED] = true;
        }
    ...
    public function hasOnlyDefaultValues(): bool
    {
            ...
            if ($this->imageurl !== 'product/default/default_product.png') {
                return false;
            }
    ...
    public function hydrate(array $row, int $startcol = 0, bool $rehydrate = false, string $indexType = TableMap::TYPE_NUM): int
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : ProductTableMap::translateFieldName('Category', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 6 + $startcol : ProductTableMap::translateFieldName('Imageurl', TableMap::TYPE_PHPNAME, $indexType)];
            $this->imageurl = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 7 + $startcol : ProductTableMap::translateFieldName('Isdeleted', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\Shop\\Models\\Product'), 0, $e);
        }
    }
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->purchasesScheduledForDeletion !== null) {
                if (!$this->purchasesScheduledForDeletion->isEmpty()) {
                    $pks = [];
                    foreach ($this->purchasesScheduledForDeletion as $entry) {
                        $entryPk = [];

                        $entryPk[0] = $this->getProductid();
                        $entryPk[1] = $entry->getPurchaseid();
                        $pks[] = $entryPk;
                    }
            ...
            if ($this->collPurchases) {
                foreach ($this->collPurchases as $purchase) {
                    if (!$purchase->isDeleted() && ($purchase->isNew() || $purchase->isModified())) {
                        $purchase->save($con);
                    }
                }
            }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(ProductTableMap::COL_IMAGEURL)) {
            $modifiedColumns[':p' . $index++]  = 'imageUrl';
        }
        if ($this->isColumnModified(ProductTableMap::COL_ISDELETED)) {
    ...
    public function buildCriteria(): Criteria
    {
        ...
        if ($this->isColumnModified(ProductTableMap::COL_IMAGEURL)) {
            $criteria->add(ProductTableMap::COL_IMAGEURL, $this->imageurl);
        }
        if ($this->isColumnModified(ProductTableMap::COL_ISDELETED)) {
            $criteria->add(ProductTableMap::COL_ISDELETED, $this->isdeleted);
        }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \Shop\Models\Product (or compatible) type.
     ...
     *
     * @param  bool $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Shop\Models\Product Clone of current object.
     * @throws \Propel\Runtime\Exception\PropelException
     */
    ...
    public function setProductPurchases(Collection $productPurchases, ?ConnectionInterface $con = null)
    {
        ...
        foreach ($productPurchasesToDelete as $productPurchaseRemoved) {
            $productPurchaseRemoved->setProduct(null);
        }
     ...
     * @phpstan-return ObjectCollection&\Traversable<ChildProductPurchase}> List of ChildProductPurchase objects
     */
    public function getProductPurchasesJoinPurchase(?Criteria $criteria = null, ?ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildProductPurchaseQuery::create(null, $criteria);
    ...
    /**
     * Initializes the collPurchases crossRef collection.
     *
     * By default this just sets the collPurchases collection to an empty collection (like clearPurchases());
     * however, you may wish to override this method in your stub class to provide setting appropriate
     * to your application -- for example, setting the initial array to the values stored in database.
     *
     * @return void
    ...
    public function initPurchases()
    {
        ...
        $this->collPurchases->setModel('\Shop\Models\Purchase');
    }

    /**
     * Checks if the collPurchases collection is loaded.
     *
     * @return bool
     */
    public function isPurchasesLoaded(): bool
    {
        return null !== $this->collPurchases;
    }

    /**
     * Gets a collection of ChildPurchase objects related by a many-to-many relationship
    ...
    /**
     * Sets a collection of Purchase objects related by a many-to-many relationship
     * to the current object by way of the product_purchase cross-reference table.
     * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
     * and new objects from the given Propel collection.
     *
    ...
    public function setPurchases(Collection $purchases, ?ConnectionInterface $con = null)
    {
        ...
        $purchasesScheduledForDeletion = $currentPurchases->diff($purchases);

        foreach ($purchasesScheduledForDeletion as $toDelete) {
            $this->removePurchase($toDelete);
        }

        foreach ($purchases as $purchase) {
            if (!$currentPurchases->contains($purchase)) {
                $this->doAddPurchase($purchase);
            }
        }
    ...
    /**
     * Gets the number of Purchase objects related by a many-to-many relationship
     * to the current object by way of the product_purchase cross-reference table.
     *
     * @param      Criteria $criteria Optional query object to filter the query
     ...
     * @param      ConnectionInterface $con Optional connection object
     *
     * @return int The number of related Purchase objects
     */
    public function countPurchases(?Criteria $criteria = null, $distinct = false, ?ConnectionInterface $con = null): int
    {
        $partial = $this->collPurchasesPartial && !$this->isNew();
        if (null === $this->collPurchases || null !== $criteria || $partial) {
    ...
    /**
     * Associate a ChildPurchase to this object
     * through the product_purchase cross reference table.
     *
     * @param ChildPurchase $purchase
     * @return ChildProduct The current object (for fluent API support)
     */
    public function addPurchase(ChildPurchase $purchase)
    {
        ...
        if (!$this->getPurchases()->contains($purchase)) {
            ...
            $this->collPurchases->push($purchase);
            $this->doAddPurchase($purchase);
        }
    ...
    protected function doAddPurchase(ChildPurchase $purchase)
    {
        ...
        if (!$purchase->isProductsLoaded()) {
            $purchase->initProducts();
            $purchase->getProducts()->push($this);
        } elseif (!$purchase->getProducts()->contains($this)) {
            $purchase->getProducts()->push($this);
        }
    ...
    /**
     * Remove purchase of this object
     * through the product_purchase cross reference table.
     *
     * @param ChildPurchase $purchase
     * @return ChildProduct The current object (for fluent API support)
    ...
    public function removePurchase(ChildPurchase $purchase)
    {
        if ($this->getPurchases()->contains($purchase)) {
            ...
            if ($purchase->isProductsLoaded()) {
                ...
                $purchase->getProducts()->removeObject($this);
            }
            ...
            $this->purchasesScheduledForDeletion->push($purchase);
        }
